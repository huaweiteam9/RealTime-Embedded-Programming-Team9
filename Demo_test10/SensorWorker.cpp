...
SensorWorker::SensorWorker(int sensorPin, QObject* parent)
    : QObject(parent), m_sensorPin(sensorPin), m_running(true), m_timer(nullptr)
{
...
void SensorWorker::process()
{
    ...
    m_timer->start(5000); // every 5 seconds
}
...
void SensorWorker::readSensor()
{
    try {
        ...
        std::this_thread::sleep_for(std::chrono::milliseconds(18));  // Short wait for readiness

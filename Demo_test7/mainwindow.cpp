    ...
    : QMainWindow(parent)
{
    ...
    // 创建 FrameGrabber 对象和线程（采集线程）
    FrameGrabber* frameGrabber = new FrameGrabber(0, 640, 640);
    QThread* grabberThread = new QThread(this);
    frameGrabber->moveToThread(grabberThread);
    connect(grabberThread, &QThread::started, frameGrabber, &FrameGrabber::process);
    connect(this, &MainWindow::destroyed, frameGrabber, &FrameGrabber::stop);
    connect(this, &MainWindow::destroyed, grabberThread, &QThread::quit);
    grabberThread->start();

    // 设置 DetectionWorker 中的 FrameGrabber
    worker->setFrameGrabber(frameGrabber);

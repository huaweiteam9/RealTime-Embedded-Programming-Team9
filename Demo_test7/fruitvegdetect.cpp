...
bool FruitVegDetect::detectFrame(const cv::Mat& inputFrame, cv::Mat& outputFrame, QMap<QString, int>& detectionResults)
{
    if (inputFrame.empty()) {
    ...
    for (int i = 0; i < detectionMat.rows; i++) {
        ...
        if (confidence > confThreshold) {
            int left = static_cast<int>((cx - w / 2) * inputFrame.cols / inputWidth);
            int top = static_cast<int>((cy - h / 2) * inputFrame.rows / inputHeight);
            int right = static_cast<int>((cx + w / 2) * inputFrame.cols / inputWidth);
            int bottom = static_cast<int>((cy + h / 2) * inputFrame.rows / inputHeight);
